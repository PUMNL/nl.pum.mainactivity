           ...
           {eval var=$sectionHeaderTemplate}
            <tr  class="{cycle values="odd-row,even-row"} {$row.class} crm-report" id="crm-report_{$rowid}">
                <td>
                        ...
                        <span id="minus_mainactivity_summary_report{$row.civicrm_case_id}_hide" style="display: none;">
            ...
            <tr id="mainactivity_summary_report_{$row.civicrm_case_id}_hide" class="crm-case_{$row.civicrm_case_id} crm-report" style="display: none;">
                <td></td>
                <td colspan="{$columnCount}" width="99%" class="enclosingNested crm-case_{$row.civicrm_case_id}">
                    <div id="mainactivity_summary_-report-casedetails-{$row.civicrm_case_id}"></div>
                </td>
            </tr>
        {/foreach}
...
<script type="text/javascript">
    function mainactivity_summary_reportCaseDetails( caseId, contactId)
    {
        ...
        cj.ajax({
            ...
            success : function( data ){
                cj( '#mainactivity_summary_-report-casedetails-'+caseId ).html( data );
            },
    ...
    function showCaseActivities( caseId) {
        cj('#mainactivity_summary-report-casedetails-'+ caseId).show();
        cj('#mainactivity_summary_report_'+caseId+'_hide').show();
        cj('#mainactivity_summary_report'+caseId+'_show').hide();
        cj('#minus_mainactivity_summary_report'+caseId+'_hide').show();
    }

    function hideCaseActivities( caseId) {
        cj('#mainactivity_summary-report-casedetails-'+ caseId).hide();
        cj('#mainactivity_summary_report_'+caseId+'_hide').hide();
        cj('#mainactivity_summary_report'+caseId+'_show').show();
        cj('#minus_mainactivity_summary_report'+caseId+'_hide').hide();
    }
